...
/****** Object:  View [da].[HMD_SV_CMI_bedmanagement]    Script Date: 11/3/2025 16:05:14 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [da].[HMD_SV_CMI_bedmanagement]
AS
...
SELECT YEAR(DATEADD(MONTH,3,CAST(FORMAT(DATEADD(YEAR,543,CAST([DATE] as Date)),'yyyy-MM-01') as date))) as [Fiscal_Year]
		...
		,'CMI' as [Type]
		,SUM([SUM_CMI])  / SUM(CAST([count_an] as float))  AS Result
		,SUM([SUM_CMI])  AS ตัวตั้ง
...
FROM
  (
	SELECT [DATE]
			,[Nursing_unit_code]
			,SUM([d/c_ADJUSTED_RELATIVE_WEIGHT]) AS SUM_CMI
			,COUNT([d/c_encounter_id])  AS count_an
	FROM  
		(
